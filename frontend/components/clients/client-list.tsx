...
export function ClientList({ onNewClient, onEditClient, onViewProjects, onNewProject, refreshTrigger }: ClientListProps) {
  ...
  const handleToggleAtivo = async (clienteId: number, novoStatus: boolean) => {
    try {
      if (!novoStatus) {
        ...
        // Ativar cliente - buscar dados e atualizar
        const clienteResponse = await clientesService.buscarPorId(clienteId);
  ...
  const handleExcluirCliente = async (clienteId: number) => {
    if (!confirm('Tem certeza que deseja excluir este cliente? Esta ação irá desativar o cliente.')) {
      return;
    }
  ...
  const handleBulkAction = async (action: 'ativar' | 'desativar' | 'excluir') => {
    ...
    try {
      ...
      toast({
        title: "Erro",
        description: `Não foi possível ${actionText} alguns clientes.`,
        variant: "destructive",
      });
    }
  };
